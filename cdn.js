...
const CatalogLayer = await $arcgis.import(
...
const { getCatalogLayerForLayer, isLayerFromCatalog } = await $arcgis.import(
...
const CatalogLayerView = await $arcgis.import(
  "@arcgis/core/views/layers/CatalogLayerView.js",
);
...
async function listItemCreatedFunction(event) {
  ...
  if (layer) {
    ...
    if (isLayerFromCatalog(layer) && getCatalogLayerForLayer(layer)) {
      item.actionsSections.getItemAt(0)?.push(
        new ActionButton({
...
async function setupLayerList(layerListElement) {
  ...
  const catalogSelectionWatchHandle = reactiveUtils.watch(
    () => layerListElement.catalogLayerList?.selectedItems?.at(0)?.layer,
    (layer) => {
  ...
  const selectedItemsWatchHandle = reactiveUtils.watch(
    () => layerListElement.selectedItems?.at(0)?.layer,
    (layer) => layer && handleLayerSelection(layer),
  );
...
function showAtCurrentViewExtent() {
  ...
  filterModeHandles.push(
    reactiveUtils.watch(
      () =>
        viewElement.map?.allLayers.filter((layer) => {
          if (!layer.fullExtent) {
            return false;
          }
...
function showVisible() {
  ...
  filterModeHandles.push(
    reactiveUtils.watch(
      () => viewElement.map?.allLayers.filter((layer) => layer.visible) ?? [],
      (layers) => {
        activeLayerListElement.filterPredicate = (item) => {
          if (!item || !item.layer) {
            return false;
